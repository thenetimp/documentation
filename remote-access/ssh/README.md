...
For headless setup, SSH can be enabled by placing a file named 'ssh', without any extension, onto the boot partition of the SD card.  This file must be in the boot partition when the Raspberry Pi is first booted with a new image to enable ssh on boot.
